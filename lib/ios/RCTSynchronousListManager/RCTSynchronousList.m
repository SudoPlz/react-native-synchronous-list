...
- (float) calculateViewSizeFromIndex: (int) fromIndex toIndex: (int) toIndex {
    if (fromIndex > toIndex) {
        RCTLogError(@"RNSynchronousList: toIndex should be greater than fromIndex.");
    }
...
- (void) createRows {
    ...
    if (!rowsAreCreated) { // if the rows are not created
        ...
        RCTLogWarn(@"RNSynchronousList: No need to prepareRows more than once.");
    }
}
...
/**
 *  getRealScrollOffset
 *  retrieves the real current x or y scroll value
 **/
- (float) getRealScrollOffset {
    CGPoint currentOffset = [self contentOffset];
    if (_horizontal) { // horizontal mode
        return (currentOffset.x - _contentOffsetShift);
    } else { // vertical mode
        return (currentOffset.y - _contentOffsetShift);
    }
}

- (int) getCurrentViewIndex {
    float curScrollOffset = [self getRealScrollOffset];
    if (self.dynamicViewSizes) {
        int curChildIndex = 0;
        if (curScrollOffset == 0 ) {
            return 0;
        }
        if (curScrollOffset > 0 ) { // if positive
            float progressiveItemSize = [self calculateViewSizeFromIndex:0 toIndex:1]; // calculate the first child size
            // we'll start adding children heights (or widths) until we get passed the cur scroll offset
            while (progressiveItemSize < curScrollOffset) {
                curChildIndex ++;
                progressiveItemSize = [self calculateViewSizeFromIndex:0 toIndex:curChildIndex + 1];
            }
            // once we've passed the cur scroll offset the cur child index is our position
            return curChildIndex;
        } else { // else if negative
            float progressiveItemSize = [self calculateViewSizeFromIndex:-1 toIndex:0];
            // we'll start subtracting children heights (or widths) until we get passed the cur scroll offset
            while (progressiveItemSize > curScrollOffset) {
                curChildIndex ++;
                progressiveItemSize = -[self calculateViewSizeFromIndex:0 toIndex:curChildIndex + 1];
            }
        ...
        float totalContentSize = _horizontal ? self.contentSize.width : self.contentSize.height;
        return (curScrollOffset / totalContentSize) * (int) data.count;
    }
    return 0;
